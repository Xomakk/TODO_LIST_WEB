const TODO_LIST = [
    ...
    {
        title: 'Вытащить курицу из морозилки',
        status: true
    }
]
...
function render() {
    ...
    for (let i = 0; i < TODO_LIST.length; i++) {
        ...
        const todo = `<li class="list-group-item bg-secondary text-white d-flex justify-content-between">
            <span class="${item.status ? 'text-decoration-line-through' : ''}">${item.title}</span>
            <span>
                <span class="btn btn-light" data-type="toggle" data-index="${i}">&check;</span>
